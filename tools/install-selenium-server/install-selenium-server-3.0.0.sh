...
if [ ! -d "/usr/lib/selenium" ]; then
  ...
  echo "Getting selenium server standalone 3.0.0\n"
  sudo wget http://selenium-release.storage.googleapis.com/3.0/selenium-server-standalone-3.0.0.jar
  sudo mv selenium-server-standalone-3.0.0.jar /usr/lib/selenium/selenium-server-standalone-3.0.0.jar
  sudo ln -s /usr/lib/selenium/selenium-server-standalone-3.0.0.jar /usr/lib/selenium/selenium-server-standalone.jar
